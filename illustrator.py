...
parser.add_argument(
    "--style",
    nargs="?",
    default="fitting the scene",
    help="Style description, e.g. " + ", ".join(STYLE_LIST),
)
parser.add_argument(
    "--dalle",
    choices=["2", "3"],
    default="3",
    help="Choose Dall-e version, either '2' or '3'",
)
parser.add_argument(
    "--optimized",
    action="store_true",
    default=False,
    help="Create an additional optimized prompt for Dall-e",
)
parser.add_argument(
    "--environment", type=str, help="Environment override, e.g. 'a fantasy world'"
)
parser.add_argument(
    "--reference-image",
    type=str,
    default="",
    help="Reference image for the illustration",
)
parser.add_argument(
...
visual_group.add_argument(
    ...
    choices=["high", "low"],
    default="auto",
    help="Image quality used during image to text, either 'high' or 'low'",
)

# Create dalle-3 group
dalle_3_group = parser.add_argument_group("Dall-e 3 specific settings")
dalle_3_group.add_argument(
    "--hd",
    action="store_true",
    default=False,
    help="High definition mode for more detailed images",
)
dalle_3_group.add_argument(
    "--detail",
    choices=["vivid", "natural"],
    default="vivid",
    help="Detail level, either 'vivid' or 'natural'",
)
...
parser.add_argument(
    ...
    choices=["1", "2", "3"],
    default="3",
    help="1=landscape (1792x1024) or small (256x256), 2=portrait (1024x1792) or medium (512x512), 3=square (1024x1024), default=3",
)
parser.add_argument(
    "--no-test", action="store_true", help="Disable test mode prompt hack"
)
...
if (args.image_url and args.image_file) or (args.dalle == "2" and args.hd):
...
def optimize_prompt(scene_description, style):
    completion = client.chat.completions.create(
        ...
        messages=[
            ...
            {
                "role": "user",
                "content": f"Describe this creation so DALL-E 3 can recreate it accurately as a {style} style image: {scene_description}. More emphasis on overview if provided. Only use visual facts, opinions and describe it as if it was created in the style of {style}.",
            },
        ],
    )
    return completion.choices[0].message.contentﬂﬂﬂﬂ


if args.optimized:
    scene_prompt = optimize_prompt(scene_description, args.style)
else:
    scene_prompt = (
        f"\nScene:\n{scene_description}\n\nStyle:\n{args.style}\n\n{args.scene}"
    )
